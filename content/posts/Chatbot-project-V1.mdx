...
excerpt: >
  In the ever-evolving landscape of web development, real-time applications have
  become a cornerstone for creating dynamic user experiences. Inspired by the
  potential of real-time communication and the growing influence of AI, I
  embarked on a project to build a real-time chatbot. This chatbot is designed
  not only to engage users but also to provide intelligent responses using AI.
  The project leverages a modern tech stack that includes JavaScript,
  TailwindCSS, Node.js, Express, and more.\
